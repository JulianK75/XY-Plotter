...
float inputX;                             //Globale Inputvariable fuer X-Koordinate, welche auf Richtigkeit geprueft wurde und umgerechnet wurde
float inputY;                             //Globale Inputvariable fuer Y-Koordinate, welche auf Richtigkeit geprueft wurde und umgerechnet wurde
bool pendown;                             //Boolean, welche den Stift entweder zeichnen laesst oder nicht
...
void loop()//Hauptschleife
{
  ...
  if (Serial.available() > 14 && wantCOM == true)//Wenn serielle Kommunikation moeglich und erwuenscht ist
  {
    communication();    //Springe zur Kommunikation
    control(bufferIn);  //Springe zur Richtigkeitskontrolle
  }

  if (failValue == 0 && failBusy == false) { //Wenn es keine Fehlermeldung gibt und der Manipulator nicht beschaeftigt ist
    ...
    failBusy = true;  //Manipulator ist jetzt beschaeftigt
    wantCOM = true; //Serielle Kommunikation ist jetzt erwuenscht



    moveToDiagonal( xInMm,  yInMm); //Funktionsaufruf Diagonale Bewegung zum neuen Punkt

    uiUpdate(   //Aktualisierung des Displays
      stepperX.currentPosition(),
      stepperY.currentPosition(),
      pendown,
      failValue, failCalibration, failBusy, failLimitSwitch, failPower,
      1
    );
  ...
  /* TODO Fehlermeldung Endlagenschalter
